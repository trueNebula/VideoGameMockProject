...
# https://sdi-project-v7qzptl4gq-ew.a.run.app/videogames/
...
class MultiplePlatformsView(APIView):
    serializer_class = HelpMeSerializer
    @csrf_exempt
    @api_view(['POST'])
    def bulkAdd(request):
        ...
        for item in platform_id_new_list:
            game = VideoGame.objects.get()
            game.save()

        return Response({'message': 'Games updated successfully.'})
